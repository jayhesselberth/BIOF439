...
.pull-left[
```{r b1, eval=F}
my_list <- list('a', c(2,3,5,6), head(mpg))
my_list
```
]
.pull-right[
```{r, echo=F, eval=T, ref.label='b1'}
...
```{r, include=F}
detach_pkg <- c('ggpubr','ggfortify','ggplot')
...
head(cleaned_data)
```
```{r, ref.label='ex5', eval=T, echo=F, message=F}
...
head(cleaned_data)
```
```{r, ref.label='ex6', echo=F, eval=T, message=F}
...
head(cleaned_data)
```
```{r, ref.label='ex7', echo=F,eval=T, message=F}
...
head(cleaned_data)
```
```{r, ref.label='ex8', eval=T, echo=F, message=F}
...
```{r ex9, eval=F, fig.height=3, fig.width=6, fig.pos='center'}
cleaned_data %>%
...
plt + stat_compare_means(label.y = 6) + 
  stat_compare_means(comparisons = my_comparisons) +
  labs(x = 'season_name', y = 'log10(enterococci)gth', color='season_name') + #<<
  theme_bw()+#<<
