...
#
library(tidyverse) #AD Added this
library(shiny)
library(ggplot2)
library(plotly)
library(readxl)
...
#AD The entire ingestion process above can be done with the code below:
#AD
#AD process_data <- function(d){
#AD   d %>% 
#AD     mutate(data_point = as.numeric(data_point)) %>% 
#AD     mutate(date_of_experiment = as.Date(date_of_experiment)) %>% 
#AD     mutate(biological_sample_group = ifelse(biological_sample_group == 'control','Wild Type', 'Knock Out'))
#AD }
#AD 
#AD fnames <- dir('.', pattern = 'xlsx')
#AD 
#AD for (f in fnames){
#AD   dat <- read_excel(f) %>% process_data()
#AD   objname <- str_remove(f, '.xlsx')
#AD   assign(objname, dat)
#AD }
#AD
#AD When you're finding yourself copying and pasting code repeatedly to perform
#AD the same tasks, it's better to write a function so that mistakes can happen
#AD in only one place rather than each time you cut, paste and adapt. 
...
server <- function(input, output) {
        output$scatterPlot <- renderPlotly ({
            ggplotly(ggplot(get(input$Param), 
                            aes(date_of_experiment, 
                                data_point, 
                                color = biological_sample_group, 
                                shape = sex)) +
            geom_point() +
