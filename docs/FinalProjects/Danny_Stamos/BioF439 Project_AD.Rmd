...
#AD fnames <- dir('.', pattern = 'xltx')
#AD if(!require(purrr)) install.packages('purrr')
#AD library(purrr)
#AD sportsref <- map(fnames, read_excel)
#AD years <- str_match(fnames, '\\d{4}') # Match with a four-digit number
#AD names(sportsref) <- years
#AD Combined_NFLDraft <- bind_rows(sportsref, .id = 'Year') %>% 
#AD   mutate(Year = as.numeric(Year))
...
#AD Doesn't look like you need this. Looks like To is already numeric
...
#AD Didn't know about coalesce. I'd actually go with tidyr::replace_na, if only 
#AD to not have to remember and find out what coalesce did next time. 
#AD 
#AD Combined_NFLDraft_4 <- Combined_NFLDraft_3 %>% mutate(G = replace_na(G,0), 
#AD    Career_Length = replace_na(Career_Length, 0))
...
#AD You could just change it to a factor rather than a character. 
